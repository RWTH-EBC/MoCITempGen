...
from sort_mo_model import modelica_model
...
class Check_OpenModelica(CI_config):
    ...
    def __init__(self,
        ...
        self.all_sims_dir = os.path.join(os.path.abspath("."), self.result_OM_check_result_dir, f'{self.library}.{self.package}')
    ...
    def check_OM_workflow(self,
        ...
        if self.om_options == "OM_SIM":
            ...
            self.sim_with_dymola(example_list=model_list)
            ...
            ERROR_DATA, STATS = self.compare_dym_to_om(example_list=model_list,
                                                       stats=STATS)
        else:
    ...
    def read_error_log(self):
        ...
        self.prepare_data(source_target_dict={
            self.check_log: os.path.join(self.result_OM_check_result_dir, f'{self.library}.{self.package}'),
            self.err_log: os.path.join(self.result_OM_check_result_dir, f'{self.library}.{self.package}')},
                          del_flag=True)
        return exit_var
    ...
    def simulate_examples(self,
        ...
        Returns:
        """
        self.create_path([self.all_sims_dir])
        ...
        if example_list is not None:
            ...
            for example in example_list:
                ...
                if "The simulation finished successfully" in result["messages"]:
                    ...
                    self.prepare_data(source_target_dict={result["resultFile"]: self.all_sims_dir})
                else:
    ...
    def sim_with_dymola(self, example_list: list = None):
        if example_list is not None:
            ...
            for model in example_list:
                ...
                self.prepare_data(source_target_dict={result: os.path.join(self.all_sims_dir, "dym")})
            self.dym_api.close()
            api_log = os.path.join(os.path.abspath("."), "DymolaAPI.log")
            self.prepare_data(source_target_dict={
                api_log: os.path.join(self.result_OM_check_result_dir, f'{self.library}.{self.package}')},
               del_flag=True)
        else:
            print(f'No examples to check. ')
            exit(0)

    def compare_dym_to_om(self, example_list:list = None, stats=None, with_plot=True):
        if example_list is not None:
            if stats is None:
                stats = {
                    "om": {
            ...
            dym_dir = os.path.join(self.all_sims_dir, "dym")
            plot_dir = os.path.join(self.all_sims_dir,"plots", self.package)
            _tol = 0.0001
            for model in example_list:
                continue_after_for = False
                for tool, _dir in zip(["om", "dymola"], [om_dir, dym_dir]):
                    path_mat = os.path.join(_dir, f'{model}.mat')
                ...
                om_tsd = TimeSeriesData(os.path.join(om_dir,  f'{model}.mat'))
                dym_tsd = TimeSeriesData(os.path.join(dym_dir, f'{model}.mat'))
                ...
                for c in dym_tsd.columns:
                    if c not in _col_err:
                        _n_diff_cols += 1
                if with_plot:
                    _dir = os.path.join(plot_dir, model)
                    if cols_to_plot:
                    ...
                    for col in cols_to_plot:
                        ...
                        plt.savefig(os.path.join(_dir,f'{col}.png'))
                        plt.cla()
...
if __name__ == '__main__':
    ...
    except_list = ["Warning: Conversion-annotation contains unknown element: nonFromVersion",
                   ...
                   "Warning: Requested package Modelica_DeviceDrivers"]
    ...
    if args.om_options == "DYMOLA_SIM":
        model_list = model.get_option_model(library=args.library,
                                            ...
                                            simulate_flag=True,
                                            filter_wh_flag=args.filter_wh_flag)
        OM.sim_with_dymola(example_list=model_list)
    if args.om_options == "COMPARE":
        ...
        model_list = model.get_option_model(library=args.library,
                                            ...
                                            simulate_flag=True,
                                            filter_wh_flag=args.filter_wh_flag)
        ERROR_DATA, STATS = OM.compare_dym_to_om(example_list=model_list,
                                                 stats=STATS)
    else:
